...
monte.carlo.statistic <- function(x, h, kernel = "gaussian") {
  ...
  monte.h_ns <- 1.06 * 1 * (n ^ (-1/5)) # bandwidth for this normal sample
    z <- (x-monte.mean)/sqrt(monte.sd^2+monte.h_ns)
...
```{r, eval=F}
write.csv(powers, "laplace(0,50).csv", row.names = F)
```
...
### chisq(1)
```{r}
powers <- read.csv("chisq(1).csv")
```

```{r}
h.normal <- 1.06 * 1 * (n ^ (-1/5)) # normal scale bandwidth

ggplot(powers[1:24,], aes(x = bandwidth)) +
  ...
  ggtitle("Powers for chisq(1)")
```

### chisq(5)
```{r}
powers <- read.csv("chisq(5).csv")
```
...
ggplot(powers[1:24,], aes(x = bandwidth)) +
  ...
  ylab("power") + 
  labs(color = "kernel functions") +
  ggtitle("Powers for chisq(5)")
```
### chisq(100)
```{r}
powers <- read.csv("chisq(100).csv")
```
...
ggplot(powers[1:24,], aes(x = bandwidth)) +
  ...
  ylab("power") + 
  labs(color = "kernel functions") +
  ggtitle("Powers for chisq(100)")
```

### norm(5,2)
```{r}
powers <- read.csv("norm(5,2).csv")
```
...
ggplot(powers[1:20,], aes(x = bandwidth)) +
  ...
  ylab("power") + 
  labs(color = "kernel functions") +
  ggtitle("Powers for norm(5,2)")
```

### laplace(0,50)
```{r}
powers <- read.csv("laplace(0,50).csv")
```
...
ggplot(powers[1:20,], aes(x = bandwidth)) +
  ...
  ylab("power") + 
  labs(color = "kernel functions") +
  ggtitle("Powers for laplace(0,50)")
```